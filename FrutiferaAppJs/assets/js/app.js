const STORAGE_KEY = "agronotas_registros";
        ...
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-start gap-2">
            <div>
              ...
              <div class="muted-soft small">${escapeHTML(item.nomeCientifico)}</div>
            </div>
            <span class="tag">${String(item.id).slice(-6)}</span>
          </div>
...
function onCreate(e){
  ...
  const data = el.dataPlantio.value.trim();

  if(!nomePopular || !nomeCientifico || !producao || !data){
    showError("Preencha tudo.");
    return;
  }

  const prod = Number(producao);
  if(!Number.isFinite(prod) || prod < 0){
    showError("A produção precisa ser um número válido.");
    return;
  }
